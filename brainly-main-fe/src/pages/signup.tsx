...
export function SignUp() {
  ...
  async function signup() {
    ...
    try {
      ...
      setError(e.response?.data?.msg || "User already exists or network error");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="bg-grain relative flex min-h-screen items-center justify-center bg-surface-50 px-4 dark:bg-surface-950">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-40 -left-40 h-[500px] w-[500px] rounded-full bg-amber-300/15 blur-3xl dark:bg-amber-500/10" />
        <div className="absolute -bottom-40 -right-40 h-[400px] w-[400px] rounded-full bg-sage-400/10 blur-3xl dark:bg-sage-500/5" />
      </div>
      ...
      >
        ...
        <div className="rounded-xl border border-surface-200 bg-white p-6 shadow-lg shadow-surface-900/5 dark:border-surface-800 dark:bg-surface-900 dark:shadow-none">
          ...
          >
            ...
            <Input ref={usernameRef} label="Username" placeholder="Choose a username" />
            <Input ref={passwordRef} type="password" label="Password" placeholder="Min 6 characters" />
        ...
        <p className="mt-5 text-center text-sm text-surface-500 dark:text-surface-400">
          ...
          <a href="/signin" className="font-semibold text-amber-600 hover:text-amber-500 dark:text-amber-400 dark:hover:text-amber-300 transition-colors">
            Sign in
          </a>
        </p>
      </motion.div>
